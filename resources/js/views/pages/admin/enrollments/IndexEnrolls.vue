    ...
    <div class="flex flex-col md:flex-row gap-12" v-else>
        <div class="w-full">
            <div class="card flex flex-col gap-4">
                <div class="font-semibold text-xl">Matriculas</div>
                    <DataTable
                    ...
                    <template #header>
                        <div class="flex justify-between">
                            <router-link to="/admin/enrollments/create">
                    ...
                    <Column header="Estudante" style="min-width: 12rem">
                        <template #body="{ data }">
                            {{ data.user.name }}
                        </template>
                    </Column>
                    <Column header="Curso" style="min-width: 12rem">
                        <template #body="{ data }">
                            {{ data.course.title }}
                        </template>
                    </Column>
                    <Column header="Matricula" style="min-width: 12rem">
                        <template #body="{ data }">
                            {{ moment(data.enrolled_at).format('DD-MM-YYYY H:mm') }}
                        </template>
                    </Column>
                    <Column header="Expira" style="min-width: 12rem">
                        <template #body="{ data }">
                            {{ moment(data.expires_at).format('DD-MM-YYYY H:mm') }}
                        </template>
                    </Column>
                    ...
                    <template #body="{ data }">
                        <router-link class="m-3" :to="'/admin/enrollments/' + data.id + '/edit'"><i class="pi pi-file-edit"></i></router-link>  <router-link class="m-3" :to="'/admin/enrollments/' + data.id"><i class="pi pi-eye"></i></router-link>
